import{B as e,E as t,G as n,J as r,K as i,L as a,T as o,U as s,_ as c,at as l,m as u,ot as d,w as f,x as p}from"../chunks/H09IMVH7.js";import{o as m}from"../chunks/D4HAnt_U.js";import"../chunks/xihTtKlq.js";import"../chunks/D8EN8fFf.js";import{t as h}from"../chunks/D_gJAAal.js";import{t as g}from"../chunks/BF3MJUBc.js";import{t as _}from"../chunks/Ciszbs-_.js";var v=t(`<section><header class="major"><h1 style="font-size: 3em;">Transactions</h1></header> <p>In Node.js, Nymph supports nested transactions. Nothing will be permanently
    ...
    they were started, or you might run into trouble.</p> <header class="major"><h2>Manually Creating Transactions</h2></header> <p><code>Nymph</code> has the following instance methods for manually managing transactions.</p> <ul><li><code>startTransaction</code> - Start a named transaction.</li> <li><code>commit</code> - Commit a named transaction.</li> <li><code>rollback</code> - Rollback a named transaction.</li> <li><code>inTransaction</code> - Check if Nymph is currently running within a transaction.</li></ul> <p><code>startTransaction</code> will return a new, transactional instance of Nymph.
    For the rest of the transaction, until it is committed or rolled back, you should
    use this instance. It will be tied to a specific connection to the database.</p> <header class="major"><h2>The <code>transactional</code> Decorator</h2></header> <p>Nymph also exports a decorator, <code>transactional</code>, that can be used
    ...
    string.</p> <header class="major"><h2>The <code>transaction</code> Helper Function</h2></header> <p>Nymph also exports a <code>transaction</code> helper function that accepts
    ...
    before promise resolution/rejection and is passed the original instance of <code>Nymph</code>. This is helpful if you need a transaction around only
    part of a method, or something that isn't an entity instance method.</p> <header class="major"><h2>Handling Transactional Instances</h2></header> <p>The transactional instance of Nymph has its own set of classes. You can use
    its <code>getEntityClass</code> method to get the proper classes for that
    instance of Nymph. This is only stictly necessary for <code>factory</code> and other static methods, since <code>Nymph</code>'s <code>getEntities</code> and <code>getEntity</code> methods will always use the correct classes to instantiate
    entities, even if you pass in one from a higher instance.</p> <p>When you start a new transaction, entity classes and instances retrieved
    from that transactional Nymph instance will have that instance within their
    static <code>nymph</code> property and instance <code>$nymph</code> property. As long as the code in your
    entities always uses these instances, every query and change should occur within
    that transaction.</p> <p>Since it is <strong>very</strong> important to use the right instance when
    you are using transactions, you should use the <code>$setNymph(nymph)</code> method on your entities within transactions. When you are done with the transaction,
    ...
    functions or the <code>transaction</code> helper function.</p> <header class="major"><h2>Transaction Support</h2></header> <p>Not all databases that Nymph supports (specifically, MySQL with an engine
    ...
    configuration that supports transactions.</p> <header class="major"><h2>Examples</h2></header> <p>Here is an example of a class that uses a transaction to delete all of its
    ...
    There is an example for each method of creating this transaction.</p> <!> <p>The <code>$setNymph</code> method is used to make sure the entity and all
    referenced entities use the transactional Nymph instance. The <code>tnymph</code> Nymph instance is used during the transaction. You don't
    ...
    methods will not, however.</p></section> <section class="page-steps"><a class="button" style="margin: .5em;">Previous: UIDs</a> <a class="button" style="margin: .5em;">Next: Full Text Search</a></section>`,1);function y(t){var y=v();c(`12sv9mp`,e=>{var t=o(),n=i(t);p(n,()=>_),a(()=>{s.title=`Transactions - User Guide - Nymph.js`}),f(e,t)});var b=i(y),x=r(n(b),38);h(x,{get language(){return g},code:`import {
...
`}),l(2),d(b);var S=r(b,2),C=n(S),w=r(C,2);d(S),e(()=>{u(C,`href`,`${m??``}/user-guide/uids`),u(w,`href`,`${m??``}/user-guide/full-text-search`)}),f(t,y)}export{y as component};