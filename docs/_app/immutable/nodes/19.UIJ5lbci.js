import{B as e,E as t,G as n,J as r,K as i,L as a,T as o,U as s,_ as c,at as l,m as u,ot as d,w as f,x as p}from"../chunks/H09IMVH7.js";import{o as m}from"../chunks/B53uXem5.js";import"../chunks/xihTtKlq.js";import"../chunks/D8EN8fFf.js";import{t as h}from"../chunks/D_gJAAal.js";import{t as g}from"../chunks/BF3MJUBc.js";import{t as _}from"../chunks/Ciszbs-_.js";var v=t(`<section><header class="major"><h1 style="font-size: 3em;">Defining Entities</h1></header> <p>To create a new type of data object in Nymph, you extend the <code>Entity</code> class. This is equivalent to creating a new table in a relational database.
    ...
    determine where to search for entities. If you don't provide a class, the <code>Entity</code> class and the "entity" etype will be used.</p> <p>The <code>$clientEnabledMethods</code> property and the <code>clientEnabledStaticMethods</code> static property in Node.js determine which methods and static methods can be called
    from the client using <code>$serverCall</code> and <code>serverCallStatic</code>. In the client class, the <code>return await this.$serverCall('$archive', []);</code> statement takes advantage
    of this feature.</p> <p>You can define a <code>$updateDataProtection()</code> method on your entity. You can then call this method from the <code>constructor()</code> and the <code>$putData()</code> method to update the security related
    instance properties, <code>$privateData</code>, <code>$protectedData</code>, <code>$allowlistData</code>, <code>$protectedTags</code>, <code>$allowlistTags</code>, and <code>$clientEnabledMethods</code>, based
    on the currently logged in user.</p> <p>To geth the currently logged in user, you can use <code>enforceTilmeld(this)</code> to get the Tilmeld instance, then <code>tilmeld.currentUser</code>. In the
    client, you would use <code>await User.current()</code> instead, since there is
    no Tilmeld instance.</p> <p>On both the Node.js class and the client class, the class name is set in the <code>class</code> static property. This class name should match on each side. It is how Nymph
...
const Todo = nymph.addEntityClass(TodoClass);`}),d(x);var C=r(x,2),w=r(n(C),2);h(w,{get language(){return g},code:`import { Entity } from '@nymphjs/client';
...
// It is important to use the Todo class returned by addEntityClass (or
// getEntityClass) because it has a reference to the instance of Nymph.
const Todo = nymph.addEntityClass(TodoClass);`}),d(C),l(18),d(b);var T=r(b,2),E=n(T),D=r(E,2);d(T),e(()=>{u(E,`href`,`${m??``}/user-guide/entity-class`),u(D,`href`,`${m??``}/user-guide/uids`)}),f(t,y)}export{y as component};